...
export function Checkout() {
  ...
  useDocumentTitle(t('checkout.eyebrow'));
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      if (data.valid) {
        ...
        showToast(`${t('checkout.code')} ${data.code} ${t('checkout.couponApplied')}: −${data.percentOff}%`);
      } else {
        setCoupon(null);
        showToast(t('checkout.couponInvalid'));
      }
    } catch {
      showToast(t('checkout.couponError'));
    } finally {
  ...
  if (!loading && !user) {
    return (
      <div className="pt-40 pb-32 text-center px-6">
        <span className="eyebrow">{t('checkout.oneMore')}</span>
        <h1 className="font-display text-4xl md:text-5xl mt-4 mb-6 uppercase">{t('checkout.signInTitle')}</h1>
        <p className="text-stone max-w-sm mx-auto mb-10">{t('checkout.signInSub')}</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <Link to="/login?redirect=/checkout" className="btn-ink">{t('checkout.signIn')}</Link>
          <Link to="/register?redirect=/checkout" className="btn-outline">{t('checkout.createAccount')}</Link>
        </div>
      </div>
    );
  }
  ...
  if (lines.length === 0) {
    return (
      <div className="pt-40 pb-32 text-center px-6">
        <h1 className="font-display text-4xl md:text-5xl mb-6 uppercase">{t('checkout.emptyTitle')}</h1>
        <Link to="/shop" className="btn-ink">{t('checkout.explore')}</Link>
      </div>
    );
  }
    ...
    <div className="pt-28 md:pt-36 pb-32">
      <div className="max-w-editorial mx-auto px-5 md:px-10 grid lg:grid-cols-[1.2fr_1fr] gap-16">
        ...
        <div>
          ...
          <h1 className="font-display text-4xl md:text-5xl mt-3 mb-10 uppercase">{t('checkout.title')}</h1>
        ...
        <div className="lg:border-l lg:border-bone/10 lg:pl-12">
          <span className="eyebrow">{t('checkout.yourOrder')}</span>
              ...
              <li key={l.variantId} className="flex gap-4">
                ...
                <div className="flex-1 flex justify-between">
                  <div>
                    ...
                    <p className="text-xs text-stone mt-1">{t('cart.size')} {l.size} · ×{l.quantity}</p>
                  </div>
                  <span className="text-sm">{price(l.priceCents * l.quantity)}</span>
                </div>
              </li>
            ))}
          </ul>
            ...
            {coupon ? (
              <div className="flex items-center justify-between text-sm">
                <span className="text-bone">
                  {t('checkout.code')} <span className="uppercase tracking-wide">{coupon.code}</span> · −{coupon.percentOff}%
                </span>
                <button
          ...
          <div className="mt-6 pt-6 border-t border-bone/10 space-y-2 text-sm">
            ...
            {discount > 0 && (
              <div className="flex justify-between text-stone">
                ...
                <span className="text-clay">−{price(discount)}</span>
              </div>
            )}
            <div className="flex justify-between text-stone">
              <span>{t('cart.shipping')}</span>
              <span className="text-bone">{shipping === 0 ? t('cart.free') : price(shipping)}</span>
            </div>
            {shipping > 0 && (
              <p className="text-[11px] text-clay pt-1">
                {t('checkout.freeOver', { free: price(config.freeShippingMinCents) })}
              </p>
            )}
          </div>
          <div className="mt-4 pt-4 border-t border-bone/10 flex justify-between items-center">
            ...
            <span className="text-2xl">{price(subtotal - discount + shipping)}</span>
          </div>
        </div>
      </div>
    </div>
