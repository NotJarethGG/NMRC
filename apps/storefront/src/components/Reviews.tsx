...
export function ReviewsSection({ product, slug }: { product: Product; slug: string }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(mine ? t('reviews.updated') : t('reviews.thanks'));
    } catch {
      showToast(t('reviews.failed'));
    } finally {
      setSending(false);
    }
  };
    ...
    <section id="reviews" className="px-5 md:px-0 mt-24 md:mt-32">
      <Reveal className="mb-10">
        ...
        <div className="flex flex-wrap items-end gap-x-6 gap-y-2 mt-2">
          <h2 className="font-display text-3xl md:text-4xl uppercase">{t('reviews.title')}</h2>
          {summary && (
            <span className="flex items-center gap-2 pb-1">
              <Stars value={summary.avg} />
              <span className="text-sm text-stone">
                {summary.avg.toFixed(1)} · {summary.count}{' '}
                {summary.count === 1 ? t('reviews.review') : t('reviews.reviews')}
              </span>
            </span>
          )}
        </div>
                ...
                <li key={r.id} className="border-b border-bone/10 pb-6">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] text-stone">
                      {new Date(r.createdAt).toLocaleDateString(locale === 'en' ? 'en-US' : 'es-CR', {
                        day: '2-digit',
                        month: 'short',
                        year: 'numeric',
                      })}
                  ...
                  <p className="text-[11px] uppercase tracking-wide text-stone mt-3">
                    {r.user?.name ?? t('reviews.anon')}
                  </p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-stone text-sm">{t('reviews.empty')}</p>
          )}
        </div>
        ...
        <div className="lg:border-l lg:border-bone/10 lg:pl-12">
          {user ? (
            <form onSubmit={submit} className="max-w-sm">
              <p className="eyebrow mb-4">{mine ? t('reviews.editTitle') : t('reviews.leaveTitle')}</p>
              ...
              <button type="submit" disabled={sending} className="btn-ink w-full mt-5">
                {sending ? t('reviews.sending') : mine ? t('reviews.update') : t('reviews.submit')}
              </button>
            </form>
          ) : (
            <div className="max-w-sm">
              <p className="eyebrow mb-3">{t('reviews.leaveTitle')}</p>
              <p className="text-sm text-stone mb-6">{t('reviews.signInPrompt')}</p>
